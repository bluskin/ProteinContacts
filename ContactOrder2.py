...
"""
This program calculates relative contact order and relative contact breadth
from a PDB file.
It disregards all hydrogen atoms.
The command-line argument 'threshold' should be in Angstroms.
If multiple models are present in a PDB file, it will only consider the first.
The output is written to a CSV file in the same folder as the program.
Written by Dan Baum and Ben Luskin, June, 2011.
"""
...
#Calculates the distance between (x1, y1, z1) and (x2, y2, z2).
...
#Finds atomic distances less than the given threshold. Returns a list of atomic contacts and a list
#containing only the residue numbers that are in contact.
...
#Counts how many times each residue contacts every other residue.
...
def calcRCO(dictOfContacts):
    ...
    for residue in dictOfContacts:
        ...
        try:
            dictOfRCOs[residue] = 1.0 * sigma / (L * Ni)
...
#Calculates the average RCO or RCB for 10%, 20%, and 30% terminus.
def calcAverages(RCO):
    #Contains average RCO or RCB for 10%, 20%, and 30% N terminus.
    NTerminusAverages = []
    #Contains average RCO or RCB for whatever residues aren't in the termini.
    middleAverages = []
    #Contains average RCO or RCB for 10%, 20%, and 30% C terminus.
    CTerminusAverages = []
    #This number will be used later while calculating secondary structure.
    global tenPercent
    tenPercent = int(0.1 * len(RCO.keys()))
    #For each of these, listOfValues contains the RCO or RCB values.
    listOfValues = []
...
#Calculates the fraction of each terminus that is a helix or sheet.
...
def main():
    ...
    printMatrix = [['% terminus','10%', '20%', '30%'], NTerminusRCO, middleRCO, CTerminusRCO, NTerminusRCB, middleRCB, CTerminusRCB, [], ['# Residues', L], []]
